...
def get_method_parameters(method: str) -> Dict[str, Any]:
    """
    Get the canonical parameters for a specific decoding method.

    This enforces strict parameter isolation - each method gets only its
    designated parameters while others are set to neutral defaults.
    ...
    if method == "greedy":
        ...
        return {
            "base_method": "beam",
            "temperature": 1.0,  # Default for beam search
            "top_p": 1.0,        # Disabled for beam search
            "top_k": None,       # Disabled for beam search
            "num_beams": beam_size,
            "do_sample": False
        }
        ...
        # Extract p value (e.g., "nucleus_0.95" -> 0.95, "nucleus" -> 0.95)
        top_p = 0.95  # Default
        if "_" in method:
            try:
        ...
        return {
            ...
            "temperature": 1.0,  # Default temperature
            "top_p": top_p,
            "top_k": None,       # Disabled for nucleus sampling
            "num_beams": 1,      # No beam search
            "do_sample": True
        }

    elif method == "top_k" or method.startswith("top_k_"):
        # Extract k value (e.g., "top_k_50" -> 50, "top_k" -> 50)
        top_k = 50  # Default
        if method.startswith("top_k_"):  # Only for parameterized versions
            try:
        ...
        return {
            "base_method": "contrastive",
            "temperature": 1.0,  # Default
            "top_p": 1.0,        # Default
            "top_k": 5,          # Default for contrastive search
            "num_beams": 1,      # No beam search
            "do_sample": False,
