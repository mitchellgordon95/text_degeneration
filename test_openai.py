#!/usr/bin/env python3
"""Test OpenAI models with all decoding methods."""
...
def test_openai_model(model_name: str, num_samples: int = 3) -> bool:
    """Test a single OpenAI model with all decoding methods."""
...
def main(num_samples: int = 3) -> bool:
    ...
    print("\n" + "=" * 70)
    print("OpenAI Models Test Suite")
    print("=" * 70)

    all_success = True
    for model_name in models:
        success = test_openai_model(model_name, num_samples)
    ...
    if all_success:
        print(f"✅ All {len(models)} OpenAI models tested successfully")
    else:
        print(f"❌ Some OpenAI models had failures")
